                    ...
                    if "data" in secret:
                        for key, value in secret["data"].items():
                            try:
                                ...
                                print_success(
                                    f"Secret {secret['metadata']['name']}.{key}: {decoded}"
                                )
                            except Exception as e:
    ...
    for service in services:
        ...
        with open(alembic_ini, "r") as f:
            ...
            if "${DATABASE_URL}" not in ini_content:
                print_error(
                    f"{service} alembic.ini doesn't use DATABASE_URL environment variable"
                )
                return False
        ...
        if result and result.returncode == 0:
            ...
            print_warning(
                f"{service} Alembic check failed (expected if database not available)"
            )
...
def test_deployment_manifests():
    ...
    project_root = Path(__file__).parent.parent.parent
    deployments_path = (
        project_root / "infrastructure" / "kubernetes" / "base" / "deployments"
    )
            ...
            for doc in docs:
                if doc and doc.get("kind") == "Deployment":
                    ...
                    if not migration_container:
                        print_error(
                            f"Migration init container not found in {deployment_file}"
                        )
                        return False
                    ...
                    if not any("alembic" in str(cmd) for cmd in command):
                        print_error(
                            f"Migration container doesn't run alembic in {deployment_file}"
                        )
                        return False

                    print_success(
                        f"Deployment {deployment_file} has valid migration init container"
                    )
        ...
        try:
            ...
            for doc in docs:
                if doc and doc.get("kind") == "Deployment":
                    ...
                    for container_list, container_type in [
                        (init_containers, "init"),
                        (main_containers, "main"),
                    ]:
                        for container in container_list:
                            ...
                            missing_vars = [
                                var for var in required_vars if var not in found_vars
                            ]
                            if missing_vars:
                                print_error(
                                    f"Missing env vars in {container_type} container: {missing_vars}"
                                )
                                return False

                            print_success(
                                f"{deployment_file} {container_type} container has all required env vars"
                            )
