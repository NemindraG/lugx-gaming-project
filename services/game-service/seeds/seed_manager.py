...
class SeedVersionController:
    ...
    async def rollback_to_version(self, target_version: str):
        ...
        await self.apply_seed_version(target_version, f"Rollback from {current_version}")
    ...
    async def clear_seed_data(self):
        ...
        async with self.async_session() as session:
            ...
            tables = ['reviews', 'inventory', 'game_categories', 'games', 'categories', 'publishers']
    ...
    async def status(self):
        ...
        # Show table counts
        async with self.async_session() as session:
            tables = ['publishers', 'categories', 'games', 'game_categories', 'inventory', 'reviews']
            
            for table in tables:
                result = await session.execute(text(f"SELECT COUNT(*) FROM game_service.{table}"))
        ...
        for version_info in history[:5]:  # Show last 5 versions
            print(f"  • {version_info['version']} - {version_info['applied_at']} - {version_info['description']}")
