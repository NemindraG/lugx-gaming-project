...
from app.models.game import Base, Publisher, Category, Game, GameCategory, Inventory, Review

# Database configuration
DATABASE_URL = os.getenv('DATABASE_URL', 'postgresql+asyncpg://game_service:game_secure_password_2024@localhost:5432/lugx_games')
...
CATEGORIES_DATA = [
    # Main categories
    {'name': 'Action', 'slug': 'action', 'description': 'Fast-paced games requiring quick reflexes', 'display_order': 1},
    {'name': 'Adventure', 'slug': 'adventure', 'description': 'Story-driven exploration games', 'display_order': 2},
    {'name': 'RPG', 'slug': 'rpg', 'description': 'Role-playing games with character progression', 'display_order': 3},
    {'name': 'Strategy', 'slug': 'strategy', 'description': 'Games requiring tactical thinking', 'display_order': 4},
    {'name': 'Simulation', 'slug': 'simulation', 'description': 'Games that simulate real-world activities', 'display_order': 5},
    {'name': 'Sports', 'slug': 'sports', 'description': 'Sports simulation and arcade games', 'display_order': 6},
    {'name': 'Racing', 'slug': 'racing', 'description': 'Vehicle racing games', 'display_order': 7},
    {'name': 'Fighting', 'slug': 'fighting', 'description': 'Combat-focused games', 'display_order': 8},
]

# Sub-categories will be added with parent relationships
SUB_CATEGORIES_DATA = [
    # Action sub-categories
    {'name': 'First-Person Shooter', 'slug': 'fps', 'parent': 'action', 'display_order': 1},
    {'name': 'Third-Person Shooter', 'slug': 'tps', 'parent': 'action', 'display_order': 2},
    {'name': 'Platformer', 'slug': 'platformer', 'parent': 'action', 'display_order': 3},
    
    # Adventure sub-categories
    {'name': 'Point and Click', 'slug': 'point-and-click', 'parent': 'adventure', 'display_order': 1},
    {'name': 'Puzzle Adventure', 'slug': 'puzzle-adventure', 'parent': 'adventure', 'display_order': 2},
    ...
    {'name': 'Real-Time Strategy', 'slug': 'rts', 'parent': 'strategy', 'display_order': 1},
    {'name': 'Turn-Based Strategy', 'slug': 'turn-based', 'parent': 'strategy', 'display_order': 2},
]
        ...
        try:
            ...
            for game_slug, game in game_objects.items():
                ...
                digital_inventory = Inventory(
                    id=uuid4(),
                    game_id=game.id,
                    quantity_available=999999 if game.price > 0 else 999999,  # Unlimited for free-to-play
                    quantity_reserved=0,
            ...
            for game_slug, game in game_objects.items():
                ...
                for i in range(num_reviews):
                    ...
                    existing_reviews = len([r for r in session.new if isinstance(r, Review) 
                                          and r.game_id == game.id and r.user_id == user_id])
                    if existing_reviews == 0:
                        rating = max(1, min(5, int((game.metacritic_score or 75) / 20) + (-1 if i % 3 == 0 else 0 if i % 2 == 0 else 1)))
                        review = Review(
                            id=uuid4(),
                            game_id=game.id,
                            user_id=user_id,
                            rating=rating,
                            title=f"Great game!" if rating >= 4 else "Could be better" if rating >= 3 else "Not impressed",
