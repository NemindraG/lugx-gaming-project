...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["parent_id"], ["game_service.categories.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("name"),
        sa.UniqueConstraint("slug"),
    ...
    op.create_table(
        ...
        sa.Column(
            "screenshots", postgresql.ARRAY(sa.Text()), nullable=True, default=[]
        ),
        ...
        sa.ForeignKeyConstraint(
            ["publisher_id"], ["game_service.publishers.id"], ondelete="RESTRICT"
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("slug"),
        schema="game_service",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["category_id"], ["game_service.categories.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["game_id"], ["game_service.games.id"], ondelete="CASCADE"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "inventory_type", sa.String(length=20), nullable=False, default="digital"
        ),
    ...
    op.create_index(
        "idx_games_publisher_id", "games", ["publisher_id"], schema="game_service"
    )
    ...
    op.create_index(
        "idx_games_trending_score", "games", ["trending_score"], schema="game_service"
    )
    op.create_index("idx_games_price", "games", ["price"], schema="game_service")
    op.create_index(
        "idx_games_release_date", "games", ["release_date"], schema="game_service"
    )
    op.create_index(
    ...
    op.create_index(
        "idx_categories_parent_id", "categories", ["parent_id"], schema="game_service"
    )
    op.create_index(
    ...
    op.create_index(
        "idx_game_categories_category_id",
        "game_categories",
        ["category_id"],
        schema="game_service",
    )
    op.create_index(
        "idx_game_categories_is_primary",
        "game_categories",
        ["is_primary"],
        schema="game_service",
    )
    ...
    op.create_index(
        "idx_inventory_game_id", "inventory", ["game_id"], schema="game_service"
    )
    op.create_index(
        "idx_inventory_type", "inventory", ["inventory_type"], schema="game_service"
    )
    op.create_index(
    ...
    op.create_index(
        "idx_reviews_game_id", "reviews", ["game_id"], schema="game_service"
    )
    op.create_index(
        "idx_reviews_user_id", "reviews", ["user_id"], schema="game_service"
    )
...
def downgrade() -> None:
    ...
    for table in ["publishers", "categories", "games", "inventory", "reviews"]:
        op.execute(
            f"DROP TRIGGER IF EXISTS {table}_update_updated_at ON game_service.{table}"
        )

    op.execute(
        "DROP TRIGGER IF EXISTS games_search_vector_update ON game_service.games"
    )
