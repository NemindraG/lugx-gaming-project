...
class CategoryRepository(BaseRepository[Category]):
    ...
    async def get_root_categories(self) -> List[Category]:
        ...
        query = (
            select(Category)
            .where(and_(Category.parent_id.is_(None), Category.is_active.is_(True)))
            .order_by(Category.display_order, Category.name)
        )
    ...
    async def get_subcategories(self, parent_id: UUID) -> List[Category]:
        ...
        query = (
            select(Category)
            .where(and_(Category.parent_id == parent_id, Category.is_active.is_(True)))
            .order_by(Category.display_order, Category.name)
        )
    ...
    async def get_category_hierarchy(self) -> List[Dict[str, Any]]:
        ...
        for category in categories:
            if category.parent_id is None:
                ...
                cat_dict["children"].sort(key=lambda x: x["display_order"])  # type: ignore
                hierarchy.append(cat_dict)  # type: ignore
    ...
    async def get_categories_with_games(self, min_games: int = 1) -> List[Category]:
        ...
        query = (
            select(Category)
            .join(GameCategory, Category.id == GameCategory.category_id)
            .join(Game, GameCategory.game_id == Game.id)
            .where(and_(Category.is_active.is_(True), Game.status == "active"))
            .group_by(Category.id)
            .having(func.count(Game.id) >= min_games)
            .order_by(func.count(Game.id).desc(), Category.display_order)
        )
    ...
    async def get_category_statistics(self, category_id: UUID) -> Dict[str, Any]:
        ...
        result = await self.session.execute(
            ...
            .where(GameCategory.category_id == category_id)
        )
