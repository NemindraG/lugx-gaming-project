...
class GameRepository(BaseRepository[Game]):
    ...
    async def search(
        ...
        # Category filter
        if category_filter:
            query = query.join(Game.game_categories).join(GameCategory.category).where(
    ...
    async def get_games_by_publisher(
        ...
        query = select(Game).join(Game.publisher).where(
            and_(
        ...
        query = select(Game).where(
            and_(
                ...
                or_(
                    ...
                    # Same categories
                    Game.game_categories.any(GameCategory.category_id.in_(category_ids))
                )
            )
        ).order_by(
            Game.trending_score.desc()
    ...
    async def get_games_with_inventory(
        self, 
        inventory_type: str = 'digital',
        in_stock_only: bool = True
    ) -> List[Game]:
        """Get games with available inventory."""
        query = select(Game).join(Game.inventory).where(
    ...
    async def get_top_rated_games(self, limit: int = 20) -> List[Game]:
        ...
        query = select(Game).join(Game.reviews).where(
            and_(
                Game.status == 'active',
    ...
    async def get_game_statistics(self) -> Dict[str, Any]:
        ...
        result = await self.session.execute(
            select(
                ...
                func.count(Game.id).filter(Game.discount_percentage > 0).label('games_on_sale'),
        ...
        return {
            ...
            'average_metacritic': float(stats.average_metacritic) if stats.average_metacritic else 0
        }