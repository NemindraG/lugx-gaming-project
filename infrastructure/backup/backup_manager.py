...
class BackupManager:
    ...
    def __init__(self, config_path: str = "backup_config.json"):
        ...
        self.backup_root = Path(
            self.config.get("backup_root", "/var/backups/lugx-gaming")
        )
    ...
    async def backup_postgres(self, service_name: str) -> bool:
        ...
        try:
            ...
            backup_file = (
                self.backup_root / "postgres" / f"{service_name}_{timestamp}.sql"
            )
    ...
    async def backup_clickhouse(self, service_name: str) -> bool:
        ...
        try:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_file = (
                self.backup_root / "clickhouse" / f"{service_name}_{timestamp}.sql"
            )
            ...
            cmd = [
                ...
                db_config["database"],
                "--query",
                f"BACKUP DATABASE {db_config['database']} TO Disk('default', '{backup_file}')",
            ]
    ...
    async def backup_all(self) -> Dict[str, bool]:
        ...
        for service_name in self.config["databases"]["postgres"]:
            results[f"postgres_{service_name}"] = await self.backup_postgres(
                service_name
            )

        # Backup ClickHouse databases
        for service_name in self.config["databases"]["clickhouse"]:
            results[f"clickhouse_{service_name}"] = await self.backup_clickhouse(
                service_name
            )
...
async def main():
    ...
    parser.add_argument(
        "command", choices=["backup", "restore", "list", "health", "info"]
    )
    ...
    parser.add_argument(
        "--config", default="backup_config.json", help="Config file path"
    )
    ...
    if args.command == "backup":
        ...
        if args.service == "postgres" and args.name and args.file:
            ...
            print(
                "Restore requires --service postgres --name <service_name> --file <backup_file>"
            )
